...
<template>
	...
	<router-link to="/mol/penguinone">idfr A</router-link>&nbsp;&nbsp;
	<router-link to="/mol/serotonin">idfr B</router-link>
	<br /><br /><br /> -->
	...
	<template v-else>
		...
		<div id="content-wrap">
			...
			<div class="col-left">
				<!-- {{ sourceType }} -->
				<BreadCrumbsNot v-if="sourceType == 'identifier'" :backto="{ name: 'mol' }">
...
// - identifier: /mol/dopamine
// - molFile:    /~/dopamine.mol.json
// - molset:     /mol/~/my_mols.molset.json?index=1
const sourceType: ComputedRef<'identifier' | 'molFile' | 'molset'> = computed(() => {
	if (route.name == 'mol' || route.name == 'headless-mol') {
		return 'identifier'
	} else {
